...
public class ZipUtils {
     ...
     * @throws IOException 当解压缩过程出错时抛出
     */
    public static void upZipFileByName(File zipFile, String folderPath) throws IOException {
        File desDir = new File(folderPath);
        if (!desDir.exists()) {
            desDir.mkdirs();
        }
        ...
        try {
            fileEncode = ZipEncodeUtils.getEncode(zipFile.getAbsolutePath(), true);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ZipFile zf;
        if (fileEncode != null && !fileEncode.isEmpty() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            zf = new ZipFile(zipFile, Charset.forName(fileEncode));
        } else {
...
//            if (entry == null || !entry.getName().startsWith("lib")) {
//                continue;
//            }
...
//            }
     ...
     * @throws IOException
     */
    public static void decompressFile(String source, String folderPath) throws IOException {
